...
Download the model results (in .rds foramt) at https://doi.org/10.5281/zenodo.19021125O and save them in the subfolder "Results". Open the script "Create_figs_Extract_map_values.R" in R making sure to 1) set up the correct working directory 2) replicate the same folder structure provided in Table 1. Source the codes to draw figure and extract values needed to reproduce the maps shown in the paper. Files storing previously extracted predicted values are available in the subfolder "Results".
### Simulation
